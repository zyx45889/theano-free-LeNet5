...
def load_data():
    ...
    l=len(data_x)
    data_xx=numpy.zeros((l,36,36),dtype='float64')
    for i in range(l):
        for j in range(36):
            for k in range(36):
                data_xx[i][j][k]=data_x[i][j*36+k]
    train_set, valid_set, test_set = data_xx[0:int(l*0.6)-1],data_xx[int(l*0.6):int(l*0.8)-1],data_xx[int(l*0.8):l-1]
...
def test_cnn(learning_rate=0.05, L1_reg=0.00, L2_reg=0.0001, n_epochs=20, batch_size=20, n_hidden=500):
    test_pred=[]
    datasets = load_data()
    ...
    classifier = cnn(batch_size,rng,36,36)
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    for index in range(n_test_batches):
                        ...
                        for i in range(batch_size):
                            test_pred.append(p_y_give_x[i][1])
    ...
    y = []
    for i in range(n):
        threshold = (max(test_pred) - min(test_pred)) / n * i
        tt = 0
        tf = 0
        pos = 0
        neg = 0
        for j in range(len(test_pred)):
            if test_pred[j] > threshold:
                if test_set_y[j] == 1:
                    tt += 1
                else:
                    tf += 1
